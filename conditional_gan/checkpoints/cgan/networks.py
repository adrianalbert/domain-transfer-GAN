...
import numpy as np
from modules import ResnetBlock, CondInstanceNorm, TwoInputSequential, CINResnetBlock, CINUnetSkipConnectionBlock, UnetSkipConnectionBlock, InstanceNorm2d
...
def define_G(input_nc, output_nc, ngf, norm='instance', which_model_netG='unet',
    ...
    if which_model_netG == 'resnet':
        ...
        netG = UnetGenerator(input_nc, output_nc, num_downs, ngf, norm_layer = norm_layer, use_dropout = use_dropout, gpu_ids = gpu_ids)
...
def define_stochastic_G(nlatent, input_nc, output_nc, ngf, norm='instance',
                        which_model_netG='unet', use_dropout=False, gpu_ids=[]):
    ...
    if which_model_netG == 'resnet':
        #def __init__(self, nlatent, input_nc, output_nc, ngf=64, norm_layer=CondInstanceNorm, use_dropout=False, n_blocks=9, gpu_ids=[], padding_type='reflect'):
        netG = CINResnetGenerator(nlatent, input_nc, output_nc, ngf, norm_layer=norm_layer,
                              use_dropout=use_dropout, n_blocks=9, gpu_ids=gpu_ids)
    else:

        netG = CINUnetGenerator(nlatent, input_nc, output_nc, num_downs, ngf, norm_layer = norm_layer, use_dropout = use_dropout, gpu_ids = gpu_ids)
...
#UNet Generator and variants:


class CINUnetGenerator(nn.Module):
    """Create a Unet-generator with noise"""

    def __init__(self, nlatent, input_nc, output_nc, num_downs, ngf=64, norm_layer = CondInstanceNorm, use_dropout=False, gpu_ids = []):
        ...
        """
        super(CINUnetGenerator, self).__init__()
        self.gpu_ids = gpu_ids
        # construct unet structure
        unet_block = CINUnetSkipConnectionBlock(nlatent, ngf * 8, ngf * 8, input_nc=None, submodule=None, norm_layer=norm_layer, innermost=True)  # add the innermost layer
        for i in range(num_downs - 5):          # add intermediate layers with ngf * 8 filters
            unet_block = CINUnetSkipConnectionBlock(nlatent, ngf * 8, ngf * 8, input_nc=None, submodule=unet_block, norm_layer=norm_layer, use_dropout=use_dropout)
        # gradually reduce the number of filters from ngf * 8 to ngf
        unet_block = CINUnetSkipConnectionBlock(nlatent, ngf * 4, ngf * 8, input_nc=None, submodule=unet_block, norm_layer=norm_layer)
        unet_block = CINUnetSkipConnectionBlock(nlatent, ngf * 2, ngf * 4, input_nc=None, submodule=unet_block, norm_layer=norm_layer)
        unet_block = CINUnetSkipConnectionBlock(nlatent, ngf, ngf * 2, input_nc=None, submodule=unet_block, norm_layer=norm_layer)
        self.model = CINUnetSkipConnectionBlock(nlatent, output_nc, ngf, input_nc=input_nc, submodule=unet_block, outermost=True, norm_layer=norm_layer)  # add the outermost layer
    ...
    def forward(self, input, noise):
        if len(self.gpu_ids)>1 and isinstance(input.data, torch.cuda.FloatTensor):
            ...
            return self.model(input, 0*noise)
