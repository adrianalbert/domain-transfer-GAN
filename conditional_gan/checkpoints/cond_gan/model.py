...
class StochCondGan(object):
    """Stochastic conditional gan, has no cycle consistency
    Using Pix2Pix architecture, https://phillipi.github.io/pix2pix/"""
    ...
    def train_instance(self, real_A, real_B, prior_z_B):
        ...
        pred_true_B = self.netD_B.forward(torch.cat((real_A, real_B), 1).detach())
        loss_D_true_B = self.criterionGAN(pred_true_B, True)

        loss_D_B = (loss_D_fake_B + loss_D_true_B) * 0.5
        loss_D = loss_D_B
    ...
    def generate_visuals(self, real_A, real_B, prior_z_B):
        if self.ignore_noise:
            prior_z_B = prior_z_B.mul(0.).add(1.)
...
class StochCycleGAN(object):
    ...
    def generate_candidates_B(self, real_A, real_B, steps):
        visuals = {'real_A':real_A.data,  'real_B':real_B.data}
        for iteration in range(0, steps):
            if self.ignore_noise:
                prior_z_B = prior_z_B.mul(0.).add(1.)
            prior_z_B = Variable(real_B.data.new(real_B.size(0), self.opt.nlatent, 1, 1).normal_(0, 1),
                           volatile=True)
            temp_name = "fake_B_candidate_" + str(iteration + 1)
            fake_B = self.netG_A_B.forward(real_A, prior_z_B)
            visuals[temp_name] = fake_B
        return visuals
...
class AugmentedCycleGAN(object):
    ...
    def train_instance(self, real_A, real_B, prior_z_B):
        ...
        ##### Return dicts
        losses  = OrderedDict([('D_A', loss_D_A.item()), ('G_A', loss_G_A.item()), ('Cyc_A', loss_cycle_A.item()),
                               ('Cyc_z_B', loss_cycle_z_B.item()), ('KLD_z_B', kld_z_B.item()),
                               ('D_B', loss_D_B.item()), ('G_B', loss_G_B.item()), ('Cyc_B', loss_cycle_B.item()),
                               ('D_z_B', loss_D_z_B.item()),
                               ('P_t_A', pred_true_A.data.mean().item()), ('P_f_A', pred_fake_A.data.mean().item()),
                               ('P_t_B', pred_true_B.data.mean().item()), ('P_f_B', pred_fake_B.data.mean().item())])
        visuals = OrderedDict([('real_A', real_A.data), ('fake_B', fake_B.data), ('rec_A', rec_A.data),
                               ('real_B', real_B.data), ('fake_A', fake_A.data), ('rec_B', rec_B.data)])
    ...
    def supervised_train_instance(self, real_A, real_B, prior_z_B):
        ...
        ##### Return dicts
        losses  = OrderedDict([('S_A', loss_sup_A.item()), ('S_B', loss_sup_B.item()),
                               ('KLD_z_B', kld_z_B.item()),
                               ('D_z_B', loss_D_z_B.item()),
