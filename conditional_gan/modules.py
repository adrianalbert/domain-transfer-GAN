...
#Defining generator blocks for Unet and noise unet

class CINUnetSkipConnectionBlock(TwoInputModule):
    """Defines the Unet submodule with skip connection and CIN .
        X -------------------identity----------------------
        |-- downsampling -- CIN -- |submodule| -- upsampling -- CIN --|
    """

    def __init__(self, nlatent, outer_nc, inner_nc, input_nc=None,
                 submodule=None, outermost=False, innermost=False, norm_layer = CondInstanceNorm, use_dropout=False):
        ...
        """
        super(CINUnetSkipConnectionBlock, self).__init__()
        ...
        downnorm = norm_layer(inner_nc, nlatent)
        uprelu = nn.ReLU(True)
        upnorm = norm_layer(outer_nc, nlatent)
        ...
        if outermost:
            ...
            up = [uprelu, MergeModule(upconv, upnorm)]
            model = down + up
        else:
            upconv = nn.ConvTranspose2d(inner_nc * 2, outer_nc,
                                        kernel_size=4, stride=2,
                                        padding=1, bias=use_bias)
            down = [downrelu, MergeModule(downconv, downnorm)]
            up = [uprelu, MergeModule(upconv, upnorm)]
    ...
    def forward(self, x, noise):
        if self.outermost:
            ...
            return torch.cat([x, self.model(x, noise)], 1)
