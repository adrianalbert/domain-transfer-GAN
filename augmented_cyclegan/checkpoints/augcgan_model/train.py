...
def train_model():
    ...
    if opt.numpy_data:
        ...
        viz_dataset = UnalignedIterator(trainA, trainB, batch_size = 10)
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        for i, data in enumerate(train_dataset):
            ...
            if total_steps % opt.display_freq == 0:
                ...
                viz_visuals = model.generate_cycle(viz_real_A, viz_real_B, viz_prior_z_B)
                ...
                for index in range(0, 1):
                    ...
                    viz_ABA = nx.Graph()
                    viz_ABA.add_node('realA', image = viz_real_A)
                    viz_ABA.add_node('fakeB', image = viz_fake_B)
                    viz_ABA.add_node('recA', image = viz_rec_A)



                    viz_ABA.add_edge('realA','fakeB', r= 'G_AB')
                    viz_ABA.add_edge('fakeB', 'recA', r= 'G_BA')
                    edge_labels = nx.get_edge_attributes(viz_ABA,'r')
                    pos={'realA': np.array([-1, 0]), 'fakeB': np.array([0, 0]), 'recA': np.array([ 1,0 ])}

                    fig=plt.figure(figsize=(15,10))
                    ax=plt.subplot(111, title="ABA Cycle")
                    ax.set_aspect('auto')
                    nx.draw_networkx_edges(viz_ABA,pos,ax=ax)
                    nx.draw_networkx_edge_labels(viz_ABA, pos, edge_labels = edge_labels)
                    ...
                    for n in viz_ABA:
                        ...
                        piesize=(0.135/viz_ABA.node[n]['image'].shape[2])*viz_ABA.node[n]['image'].shape[1]
                        p2=piesize/2.0
                        ...
                        #a.title = "hi"
                        #a.set_aspect('equal')
                        image = viz_ABA.node[n]['image'].cpu().clone()
                    ...
                    #plt.imshow(image)
                    #plt.show()
                    plot_to_tensorboard(writer, "Cycle Viz A-B-A", fig, total_steps)
